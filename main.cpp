...
int busInit() {
    ...
    for (uint8_t addr = 0; addr < 4; addr++) { //write config to each I2C slave
        //printf("Writing config to slave %i\n", addr);
        if (writeReg(addr, 0x0A, &IOCON_cfg)) { //IOCON reg, IOCON.BANK at POR is 0, initial IOCON reg is 0x0A, changes to 0x05 after IOCON cfg 
            printf("IOCON WRITE FAILED\n");
            ...
            printf("GPPU/GPINTEN WRITE FAILED\n");
        }
    }
    return 0;
}
